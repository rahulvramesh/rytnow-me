...
export function QuickThoughtCard({
    ...
    const handleDeleteRecording = (recordingId: number) => {
        ...
        router.delete(
            `/quick-thoughts/${thought.id}/recordings/${recordingId}`,
            {
                preserveScroll: true,
            },
        );
    ...
    return (
        <div
            className={`relative overflow-hidden rounded-lg border bg-background p-4 transition-opacity ${isDeleting ? 'opacity-50' : ''}`}
                ...
                {thought.recordings && thought.recordings.length > 0 && (
                    <div className="mb-3 space-y-2">
                        {thought.recordings.map((recording) => (
                            ...
                            >
                                <Mic className="size-3.5 flex-shrink-0 text-muted-foreground" />
                                <audio
